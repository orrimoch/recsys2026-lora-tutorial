...
class CEGroupDataset(Dataset):
    """Holds [(query, [pos_doc, neg...], group_weight)]; tokenizes pairs lazily."""
    ...
    def __getitem__(self, i):
        ...
        enc = lambda s: self.tok.encode(s, add_special_tokens=False, truncation=True, max_length=self.max_length)
        qn = len(enc(q))
        budget = self.doc_budget_fn(qn, self.max_length, self.max_doc_tokens)
        pairs = [(q, self.truncate_fn(enc, self.tok.decode, d, budget)) for d in docs]
...
def finetune_cross_encoder(groups_train, groups_val, *, base_model, lora_cfg,
                           max_length=2048, max_doc_tokens=1100, dtype="bf16",
                           train_cfg, logger, out_dir, score_official_fn=None, val_eval_fn=None):
    """Fine-tune bge-reranker-v2-m3 with LoRA + masked listwise-softmax. Returns out_dir (adapter)."""
    ...
    tdtype = torch.bfloat16 if dtype == "bf16" else torch.float32
    model = model.to(device=dev, dtype=tdtype)
    ...
    def make_loader(groups, shuffle):
        ...
        return DataLoader(ds, batch_size=train_cfg["batch_groups"], shuffle=shuffle,
                          collate_fn=lambda b: _collate(b, tok, max_length))

    train_loader = make_loader(groups_train, True)
    opt = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=train_cfg["lr"])
    best, best_metric = None, -1.0
    step = 0
    for epoch in range(train_cfg["epochs"]):
        model.train()
        for feats, sizes, weights in train_loader:
            feats = {k: v.to(dev) for k, v in feats.items()}
            logits = model(**feats).logits.squeeze(-1)         # (sum sizes,)
            loss = masked_listwise_ce(logits, sizes, weights)
            loss.backward(); opt.step(); opt.zero_grad()
            step += 1
            if step % train_cfg.get("log_every", 50) == 0:
                logger.log({"train_loss": float(loss)}, step=step)
        # ---- validation ----
        model.eval()
        with torch.no_grad():
            vl = []
        ...
        logger.log({"val_loss": val_loss, "val_ndcg@20": val_ndcg}, step=step)
        if val_ndcg > best_metric:
            best_metric = val_ndcg
            model.save_pretrained(out_dir)                     # best adapter only
            best = out_dir
    return best
```
...
TRAIN = {"epochs": 2, "lr": 1e-4, "batch_groups": 8, "warmup": 0.05, "log_every": 50, "group_by_length": True}
